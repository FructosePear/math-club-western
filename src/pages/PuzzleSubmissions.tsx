...
const PuzzleSubmissions: React.FC = () => {
  ...
  const loadPuzzleAndSubmissions = async () => {
    ...
    try {
      ...
      // Load submissions
      const submissionsData = await potwService.getPuzzleSubmissionsForGrading(
        puzzleId
      );
  ...
  const confirmGradeChange = async () => {
    if (!currentUser || !gradeDialog.submission || !gradeDialog.newGrade)
      return;

    try {
      setUpdating(gradeDialog.submission.id!);
      await potwService.updateSubmissionGrade(
        gradeDialog.submission.id!,
        gradeDialog.newGrade,
        currentUser.uid
      );
      ...
      setSubmissions((prev) =>
        prev.map((sub) =>
          sub.id === gradeDialog.submission!.id
            ? {
                ...sub,
                grade: gradeDialog.newGrade,
                gradedAt: new Date() as any,
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
        ...
        <div className="container mx-auto px-4 py-12">
          <Alert>
            <AlertDescription>
              You don't have permission to view puzzle submissions. Contact an
              admin for access.
            </AlertDescription>
          </Alert>
        </div>
  ...
  if (!puzzle) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
        ...
        <div className="container mx-auto px-4 py-12">
          <Alert>
            <AlertDescription>Puzzle not found.</AlertDescription>
          </Alert>
        </div>
        <Footer />
      </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
      ...
      <div className="container mx-auto px-4 py-12">
        ...
        <div className="mb-8">
          ...
          <h2 className="text-xl text-gray-600 mb-4">{puzzle.title}</h2>
            ...
            {submissions.length === 0 ? (
              ...
              <div className="space-y-6">
                {submissions.map((submission) => (
                  <details
                    key={submission.id}
                    className="group border border-gray-200 rounded-lg bg-white transition-shadow hover:shadow-md"
                  >
                      ...
                      <div className="flex items-start justify-between mb-4">
                        ...
                        <div className="flex items-center space-x-2">
                          <Select
                            ...
                            onValueChange={(value) =>
                              handleGradeChange(submission, parseInt(value))
                            }
      ...
      <AlertDialog
        open={gradeDialog.isOpen}
        onOpenChange={(open: boolean) =>
          setGradeDialog((prev) => ({ ...prev, isOpen: open }))
        }
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to grade{" "}
              <strong>{gradeDialog.submission?.userName}</strong>'s submission
              as <strong>{gradeDialog.newGrade}/5</strong>?
              <br />
              <br />
              Current grade:{" "}
              {gradeDialog.submission?.grade
                ? `${gradeDialog.submission.grade}/5`
                : "Not graded"}
              <br />
          ...
          <AlertDialogFooter>
            <AlertDialogCancel onClick={cancelGradeChange}>
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
